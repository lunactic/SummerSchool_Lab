...
def binarize_page(input_image):
    ...
    # Start the execution on DIVAServices
    url = "http://divaservices.unifr.ch/api/v2/binarization/otsubinarization/1"
    payload = {"parameters": {},
               "data": [{"inputImage": "kws_ss_orig/" + input_image + ".jpg"}]}
...
def extract_words_binary(input_image):
    ...
    # Set up the DIVAServices execution
    url = "http://divaservices.unifr.ch/api/v2/kws/wordimageextraction/1"
    payload = {"parameters": {},
               "data": [{"inputImage": "kws_ss_binary/bin_" + input_image + ".jpg",
                         "pathSvg": "kws_ss_locations/" + input_image + ".svg"}]}
    headers = {'content-type': 'application/json'}
    response = json.loads(requests.request(
        "POST", url, data=json.dumps(payload), headers=headers).text)
